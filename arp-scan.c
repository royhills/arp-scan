...
usage(void) {
   ...
   fprintf(stdout, "\n");
   fprintf(stdout, "Target hosts must be specified on the command line unless the --file or\n");
   fprintf(stdout, "--localnet option is used.\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "arp-scan uses raw sockets, which requires privileges on some systems:\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "Linux with POSIX.1e capabilities support using libcap:\n");
   fprintf(stdout, "       arp-scan is capabilities aware. It requires CAP_NET_RAW in the permitted\n");
   fprintf(stdout, "       set and only enables that capability for the required functions.\n");
   fprintf(stdout, "BSD and macOS:\n");
   fprintf(stdout, "       You need read/write access to /dev/bpf*\n");
   fprintf(stdout, "Any operating system:\n");
   fprintf(stdout, "       Running as root or SUID root will work on any OS but other methods\n");
   fprintf(stdout, "       are preferable where possible.\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "Targets can be IPv4 addresses or hostnames. You can also use CIDR notation\n");
   fprintf(stdout, "(10.0.0.0/24) (network and broadcast included), ranges (10.0.0.1-10.0.0.10),\n");
   fprintf(stdout, "and network:mask (10.0.0.0:255.255.255.0).\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "Options:\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "The data type for option arguments is shown by a letter in angle brackets: \n");
   fprintf(stdout, "\n");
   fprintf(stdout, "<s> Character string.\n");
   fprintf(stdout, "<i> Decimal integer, or hex if preceeded by 0x e.g. 2048 or 0x800.\n");
   fprintf(stdout, "<f> Floating point decimal number.\n");
   fprintf(stdout, "<m> MAC address, e.g. 01:23:45:67:89:ab or 01-23-45-67-89-ab (case insensitive)\n");
   fprintf(stdout, "<a> IPv4 address e.g. 10.0.0.1\n");
   fprintf(stdout, "<h> Hex encoded binary data. No leading 0x. (case insensitive).\n");
   fprintf(stdout, "<x> Something else - see option description.\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "General Options:\n");
   fprintf(stdout, "\n--help or -h\t\tDisplay this usage message and exit.\n");
   fprintf(stdout, "\n--verbose or -v\t\tDisplay verbose progress messages.\n");
   fprintf(stdout, "\t\t\tCan be used than once to increase verbosity. Max=3.\n");
   fprintf(stdout, "\n--version or -V\t\tDisplay program version details and exit.\n");
   fprintf(stdout, "\t\t\tShows the version, license details, libpcap version,\n");
   fprintf(stdout, "\t\t\tand whether POSIX.1e capability support is included.\n");
   ...
   fprintf(stdout, "\n");
   fprintf(stdout, "Host Selection:\n");
   fprintf(stdout, "\n--file=<s> or -f <s>\tRead hostnames or addresses from the specified file\n");
   fprintf(stdout, "\t\t\tOne name or address pattern per line. Use \"-\" for stdin.\n");
   fprintf(stdout, "\n--localnet or -l\tGenerate addresses from interface configuration.\n");
   fprintf(stdout, "\t\t\tGenerates list from interface address and netmask\n");
   fprintf(stdout, "\t\t\t(network and broadcast included). You cannot use the\n");
   fprintf(stdout, "\t\t\t--file option or give targets on the command line.\n");
   fprintf(stdout, "\t\t\tUse --interface to specify the interface.\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "MAC/Vendor Mapping Files:\n");
   ...
   fprintf(stdout, "\t\t\tmicroseconds. Makes the ${rtt} field available for\n");
   ...
   fprintf(stdout, "\n");
   fprintf(stdout, "Host List Randomisation:\n");
   fprintf(stdout, "\n--random or -R\t\tRandomise the target host list.\n");
   fprintf(stdout, "\n--randomseed=<i>\tSeed the pseudo random number generator.\n");
   fprintf(stdout, "\t\t\tUseful if you want a reproducible --random order.\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "Output Timing and Retry:\n");
   ...
   fprintf(stdout, "\t\t\tMultiplies timeout by <f> for each pass.\n");
   ...
   fprintf(stdout, "\t\t\tthe packet rate. If you want to use up to a given\n");
   fprintf(stdout, "\t\t\tbandwidth it is easier to use the --bandwidth option\n");
   fprintf(stdout, "\t\t\tinstead. The interval is in milliseconds, or\n");
   fprintf(stdout, "\t\t\tmicroseconds if \"u\" is appended.\n");
   fprintf(stdout, "\n--bandwidth=<x> or -B <x> Set outbound bandwidth to <x>, default=%d.\n", DEFAULT_BANDWIDTH);
   fprintf(stdout, "\t\t\tThe value is in bits per second. Append K for\n");
   fprintf(stdout, "\t\t\tkilobits or M for megabits (decimal multiples). You\n");
   fprintf(stdout, "\t\t\tcannot specify both --interval and --bandwidth.\n");
   fprintf(stdout, "\n");
   fprintf(stdout, "DNS Resolution:\n");
   fprintf(stdout, "\n--numeric or -N\t\tTargets must be IP addresses, not hostnames.\n");
   fprintf(stdout, "\t\t\tCan reduce startup time for large target lists.\n");
   ...
   fprintf(stdout, "\n");
   fprintf(stdout, "Output ARP Packet:\n");
   ...
   fprintf(stdout, "\n");
   fprintf(stdout, "Output Ethernet Header:\n");
   fprintf(stdout, "\n--srcaddr=<m> or -S <m> Set the source Ethernet MAC address.\n");
   fprintf(stdout, "\t\t\tDefault is the interface MAC address. This sets the\n");
   fprintf(stdout, "\t\t\taddress in the Ethernet header. It does not change the\n");
   fprintf(stdout, "\t\t\taddress in the ARP packet: use --arpsha to change\n");
   fprintf(stdout, "\t\t\tthat address.\n");
   fprintf(stdout, "\n--destaddr=<m> or -T <m> Set the destination MAC address.\n");
   fprintf(stdout, "\t\t\tSets the destination address in the Ethernet\n");
   fprintf(stdout, "\t\t\theader. Default is ff:ff:ff:ff:ff:ff (broadcast)\n");
   fprintf(stdout, "\t\t\tHosts also respond if the request is sent to their\n");
   fprintf(stdout, "\t\t\tunicast address, or to a multicast address they\n");
   fprintf(stdout, "\t\t\tare listening on.\n");
   fprintf(stdout, "\n--prototype=<i> or -y <i> Sets the Ethernet protocol type, default=0x%.4x.\n", DEFAULT_ETH_PRO);
   fprintf(stdout, "\t\t\tThis sets the protocol type field in the Ethernet\n");
   fprintf(stdout, "\t\t\theader.\n");
   fprintf(stdout, "\n--llc or -L\t\tUse RFC 1042 LLC/SNAP encapsulation for 802.2 networks.\n");
   fprintf(stdout, "\t\t\tarp-scan will decode and display ARP responses in both\n");
   fprintf(stdout, "\t\t\tEthernet-II and IEEE 802.2 formats irrespective of\n");
   fprintf(stdout, "\t\t\tthis option.\n");
   fprintf(stdout, "\n--vlan=<i> or -Q <i>\tUse 802.1Q tagging with VLAN id <i>.\n");
   fprintf(stdout, "\t\t\tThe id should be in the range 0 to 4095. arp-scan will\n");
   fprintf(stdout, "\t\t\tdecode and display ARP responses in 802.1Q format\n");
   ...
   fprintf(stdout, "\t\t\tresponding hosts is less than the limit. Can be used\n");
   fprintf(stdout, "\t\t\tin scripts to check if fewer hosts respond without\n");
   fprintf(stdout, "\t\t\thaving to parse the output.\n");
   fprintf(stdout, "\n--pcapsavefile=<s> or -W <s>\tWrite received packets to pcap savefile <s>.\n");
   fprintf(stdout, "\t\t\tARP responses will be written to the specified file\n");
   fprintf(stdout, "\t\t\tas well as being decoded and displayed.\n");
   fprintf(stdout, "\n--snap=<i> or -n <i>\tSet the pcap snap length to <i>. Default=%d.\n", SNAPLEN);
   fprintf(stdout, "\t\t\tSpecifies the frame capture length, including the\n");
   fprintf(stdout, "\t\t\tEthernet header. The default is normally sufficient.\n");
   fprintf(stdout, "\n--retry-send=<i> or -Y <i> Set number of send attempts, default=%d.\n", DEFAULT_RETRY_SEND);
   fprintf(stdout, "\n--retry-send-interval=<i> or -E <i> Set interval between send attempts.\n");
   fprintf(stdout, "\t\t\tInterval is in milliseconds or microseconds if \"u\"\n");
   fprintf(stdout, "\t\t\tis appended. default=%d.\n", DEFAULT_RETRY_SEND_INTERVAL/1000);
   fprintf(stdout, "\n--padding=<h> or -A <h>\tSpecify padding after packet data.\n");
   fprintf(stdout, "\t\t\tSet padding after the ARP request to hex value <h>.\n");
   fprintf(stdout, "\n");
